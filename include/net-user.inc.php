...
function sendWOL(string $host_mac): bool
{
    ...
    $logSys->debug("checking mac \"{$host_mac}\"");
    $host_mac = str_replace([':', '-'], '', $host_mac);

    if (strlen($host_mac) % 2 !== 0) {
        $logSys->error("MAC address must be even \"{$host_mac}\"");
        return false;
    }

    $macAddressBinary = hex2bin($host_mac);
    if ($macAddressBinary === false) :
        $logSys->error("MAC address is not correct \"{$host_mac}\"");
        return false;
    endif;
    $magicPacket = str_repeat(chr(255), 6) . str_repeat($macAddressBinary, 16);
    $socket = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);
    if ($socket === false) {
        $logSys->error("Error creating socket " . socket_strerror(socket_last_error()));
        return false;
    }
    ...
    if ($result) {
        $logSys->debug("Successful sending WOL packet to {$host_mac}");
    } else {
        $logSys->debug("Failed sending WOL packet to {$host_mac}");
    }
    // Cerrar el socket
    socket_close($socket);
